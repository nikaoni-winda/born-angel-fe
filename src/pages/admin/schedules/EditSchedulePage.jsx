...
const EditSchedulePage = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [instructorsRes, servicesRes, scheduleData] = await Promise.all([
            ...
            setFormData({
                ...
                start_time: formatForInput(scheduleData.start_time),
                end_time: formatForInput(scheduleData.end_time),
    ...
    const handleServiceChange = (value) => {
        ...
        // Reset instructor when service changes (only if current instructor doesn't match new service)
        const currentInstructor = instructors.find(i => i.id === formData.instructor_id);
        const shouldResetInstructor = currentInstructor && currentInstructor.service_id !== value;

        setFormData(prev => ({
            ...prev,
            service_id: value,
            instructor_id: shouldResetInstructor ? '' : prev.instructor_id
        }));
        ...
        <AdminLayout>
            <div className="max-w-2xl mx-auto">
                <div className="mb-8 flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-polar-night">Edit Schedule</h1>
                        <p className="text-text-primary/60 text-sm">Update class timing and details</p>
                    </div>
                </div>
